...
export default class LgcyWeb extends EventEmitter {
  ...
  static toHex(val) {
    if (utils.isBoolean(val)) return LgcyWeb.fromDecimal(+val);

    if (utils.isBigNumber(val)) return LgcyWeb.fromDecimal(val);

    if (typeof val === 'object') return LgcyWeb.fromUtf8(JSON.stringify(val));

    if (utils.isString(val)) {
      if (/^(-|)0x/.test(val)) return val;

      if (!isFinite(val)) return LgcyWeb.fromUtf8(val);
    }

    let result = LgcyWeb.fromDecimal(val);
    if (result === '0xNaN') {
  ...
  static fromAscii(string, padding) {
    ...
    return (
      '0x' + Buffer.from(string, 'ascii').toString('hex').padEnd(padding, '0')
    );
  }

  static toDecimal(value) {
    return LgcyWeb.toBigNumber(value).toNumber();
  }

  static fromDecimal(value) {
    const number = LgcyWeb.toBigNumber(value);
  ...
  static fromSun(sun) {
    const trx = LgcyWeb.toBigNumber(sun).div(1_000_000);
    return utils.isBigNumber(sun) ? trx : trx.toString(10);
  }

  static toSun(trx) {
    const sun = LgcyWeb.toBigNumber(trx).times(1_000_000);
