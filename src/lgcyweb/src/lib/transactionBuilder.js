...
function toHex(value) {
  return self.lgcyWeb.address.toHex(value);
}

function fromUtf8(value) {
  return self.lgcyWeb.fromUtf8(value);
}
...
function resultManager(transaction, callback) {
  ...
  if (transaction.result && transaction.result.message) {
    return callback(this.lgcyWeb.toUtf8(transaction.result.message));
  }
...
export default class TransactionBuilder {
  constructor(lgcyWeb = false) {
    ...
    this.validator = new Validator(lgcyWeb);
  }

  sendTrx(
    to = false,
    amount = 0,
    from = this.lgcyWeb.defaultAddress.hex,
    callback = false,
  ) {
  ...
  purchaseToken(
    ...
    tokenID = false,
    amount = 0,
    buyer = this.lgcyWeb.defaultAddress.hex,
    callback = false,
  ) {
    ...
    if (!this.lgcyWeb.isAddress(issuerAddress))
    ...
    if (!this.lgcyWeb.isAddress(buyer))
  ...
  freezeBalance(
    ...
    if (
      ...
      !this.lgcyWeb.isAddress(receiverAddress)
    )
  ...
  applyForSR(
    ...
    if (!utils.isValidURL(url)) return callback('Invalid url provided');
  ...
  vote(
    votes = {},
    voterAddress = this.lgcyWeb.defaultAddress.hex,
    callback = false,
  ) {
  ...
  createSmartContract(
    options = {},
    issuerAddress = this.lgcyWeb.defaultAddress.hex,
    callback = false,
  ) {
    ...
    let { abi = false, bytecode = false, parameters = [], name = '' } = options;
    ...
    if (payable && callValue == 0)
      return callback(
        'When contract is payable, options.callValue must be a positive integer',
      );

    if (!payable && callValue > 0)
      return callback(
        'When contract is not payable, options.callValue must be 0',
      );

    if (
      !utils.isInteger(userFeePercentage) ||
      userFeePercentage < 0 ||
      userFeePercentage > 100
    )
    ...
    if (
      !utils.isInteger(originKandyLimit) ||
      originKandyLimit < 0 ||
      originKandyLimit > 10_000_000
    )
    ...
    if (
      utils.isNotNullOrUndefined(tokenValue) &&
      (!utils.isInteger(tokenValue) || tokenValue < 0)
    )
      return callback('Invalid options.tokenValue provided');

    if (
      utils.isNotNullOrUndefined(tokenId) &&
      (!utils.isInteger(tokenId) || tokenId < 0)
    )
      return callback('Invalid options.tokenValue provided');
    ...
    if (typeof constructorParams !== 'undefined' && constructorParams) {
      ...
      for (let i = 0; i < parameters.length; i++) {
        ...
        if (type == 'address') value = toHex(value).replace(/^(30)/, '0x');
    ...
    if (utils.isNotNullOrUndefined(tokenId)) args.token_id = parseInt(tokenId);
  ...
  _triggerSmartContract(
    ...
    if (issuerAddress !== false && !this.lgcyWeb.isAddress(issuerAddress))
      return callback('Invalid issuer address provided');
    ...
    this.lgcyWeb.fullNode
      .request('wallet/triggerconstantcontract', args, 'post')
      .then((transaction) => resultManager(transaction, callback))
      .catch((err) => callback(err));
  }
  ...
  createToken(
    ...
    if (
      !utils.isInteger(freeBandwidthLimit) ||
      freeBandwidthLimit < 0 ||
      (freeBandwidth && !freeBandwidthLimit)
    )
      return callback('Invalid free bandwidth limit provided');

    if (
      !utils.isInteger(frozenAmount) ||
      frozenAmount < 0 ||
      (!frozenDuration && frozenAmount)
    )
      return callback('Invalid frozen supply provided');

    if (
      !utils.isInteger(frozenDuration) ||
      frozenDuration < 0 ||
      (frozenDuration && !frozenAmount)
    )
    ...
    if (
      ...
      (!utils.isInteger(precision) || precision <= 0 || precision > 18)
    )
    ...
    const data = {
      ...
      lgcy_num: parseInt(trxRatio), //lgcy_num
  ...
  voteProposal(
    ...
    if (!this.lgcyWeb.isAddress(voterAddress))
  ...
  createTRXExchange(
    ...
    tokenBalance,
    trxBalance,
    ownerAddress = this.lgcyWeb.defaultAddress.hex,
    callback = false,
  ) {
    ...
    if (!this.lgcyWeb.isAddress(ownerAddress))
      return callback('Invalid address provided');
    ...
    if (
      !utils.isInteger(tokenBalance) ||
      tokenBalance <= 0 ||
      !utils.isInteger(trxBalance) ||
      trxBalance <= 0
    )
      return callback('Invalid amount provided');
  ...
  createTokenExchange(
    ...
    if (
      !utils.isInteger(firstTokenBalance) ||
      firstTokenBalance <= 0 ||
      !utils.isInteger(secondTokenBalance) ||
    ...
    this.lgcyWeb.fullNode
      .request(
        'wallet/exchangetransaction',
        {
          owner_address: toHex(ownerAddress),
          exchange_id: parseInt(exchangeID),
          token_id: this.lgcyWeb.fromAscii(tokenName),
  ...
  checkPermissions(permissions, type) {
    if (permissions) {
      ...
      for (let key of permissions.key) {
        if (
          !this.lgcyWeb.isAddress(key.address) ||
