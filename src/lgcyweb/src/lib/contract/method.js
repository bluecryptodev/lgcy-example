...
export default class Method {
  ...
  onMethod(...args) {
    ...
    args.forEach((arg, index) => {
      if (types[index] == 'address')
        args[index] = this.lgcyWeb.address.toHex(arg).replace(/^(30)/, '0x');

      if (types[index] == 'address[]') {
        args[index] = args[index].map((address) => {
          return this.lgcyWeb.address.toHex(address).replace(/^(30)/, '0x');
        });
      }
    });
    ...
    return {
      call: (...methodArgs) => {
        ...
        this._call(types, args, ...methodArgs);
      },
  ...
  async _call(types, args, options = {}, callback = false) {
    ...
    this.lgcyWeb.transactionBuilder.triggerSmartContract(
      this.contract.address,
      this.functionSelector,
      options,
      parameters,
      options.from ? this.lgcyWeb.address.toHex(options.from) : false,
      (err, transaction) => {
          ...
          if (len === 0 || len % 64 === 8) {
            ...
            if (len !== 0) {
              ...
              for (let i = 0; i < len - 8; i += 64) {
                msg2 += this.lgcyWeb.toUtf8(chunk.substring(i, i + 64));
              }
              msg += msg2
                .replace(/(\u0000|\u000b|\f)+/g, ' ')
                .replace(/ +/g, ' ')
  ...
  async _send(
    ...
    try {
      const address = privateKey
        ? this.lgcyWeb.address.fromPrivateKey(privateKey)
        : this.lgcyWeb.defaultAddress.base58;
      const transaction =
        await this.lgcyWeb.transactionBuilder.triggerSmartContract(
          this.contract.address,
          this.functionSelector,
          options,
          parameters,
          this.lgcyWeb.address.toHex(address),
        );
      ...
      const signedTransaction = await this.lgcyWeb.trx.sign(
      ...
      const broadcast = await this.lgcyWeb.trx.sendRawTransaction(
        signedTransaction,
      );
      ...
      if (broadcast.code)
        return callback({
          ...
          message: this.lgcyWeb.toUtf8(broadcast.message),
        });
      ...
      const checkResult = async (index = 0) => {
        ...
        const output = await this.lgcyWeb.trx.getTransactionInfo(
        ...
        if (output.result && output.result == 'FAILED') {
          return callback({
            error: this.lgcyWeb.toUtf8(output.resMessage),
            transaction: signedTransaction,
            output,
          });
        }
  ...
  async _watch(options = {}, callback = false) {
    ...
    const getEvents = async () => {
      try {
        ...
        const events = await this.lgcyWeb.event.getEventsByContactAddress(
          this.contract.address,
          params,
        );
