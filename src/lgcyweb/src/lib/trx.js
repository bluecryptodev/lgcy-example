...
import { toUtf8Bytes, recoverAddress, keccak256, SigningKey } from 'ethers';
...
export default class Trx {
  ...
  _parseToken(token) {
    return {
      ...token,
      name: this.lgcyWeb.toUtf8(token.name),
      abbr: token.abbr && this.lgcyWeb.toUtf8(token.abbr),
      description: token.description && this.lgcyWeb.toUtf8(token.description),
      url: token.url && this.lgcyWeb.toUtf8(token.url),
    };
  }
  ...
  getBlock(block = this.lgcyWeb.defaultBlock, callback = false) {
    if (utils.isFunction(block)) {
      callback = block;
      block = this.lgcyWeb.defaultBlock;
    }
  ...
  getTransactionsToAddress(
    address = this.lgcyWeb.defaultAddress.hex,
    limit = 30,
    offset = 0,
    callback = false,
  ) {
    ...
    address = this.lgcyWeb.address.toHex(address);
  ...
  async getTransactionsRelated(
    ...
    if (!this.lgcyWeb.isAddress(address))
  ...
  getAccount(address = this.lgcyWeb.defaultAddress.hex, callback = false) {
    if (utils.isFunction(address)) {
      callback = address;
      address = this.lgcyWeb.defaultAddress.hex;
    }
  ...
  getBalance(address = this.lgcyWeb.defaultAddress.hex, callback = false) {
    if (utils.isFunction(address)) {
      callback = address;
      address = this.lgcyWeb.defaultAddress.hex;
    }
  ...
  getBandwidth(address = this.lgcyWeb.defaultAddress.hex, callback = false) {
    if (utils.isFunction(address)) {
      callback = address;
      address = this.lgcyWeb.defaultAddress.hex;
    }
    ...
    this.lgcyWeb.fullNode
      ...
      .then(
        ({ freeNetUsed = 0, freeNetLimit = 0, NetUsed = 0, NetLimit = 0 }) => {
  ...
  getTokenFromID(tokenID = false, callback = false) {
    ...
    this.lgcyWeb.fullNode
      .request(
        ...
        {
          value: this.lgcyWeb.fromUtf8(tokenID),
        },
        'post',
      )
      .then((token) => {
  ...
  getContract(contractAddress, callback = false) {
    ...
    contractAddress = this.lgcyWeb.address.toHex(contractAddress);
  ...
  async verifyMessage(
    message = false,
    signature = false,
    address = this.lgcyWeb.defaultAddress.base58,
    useTronHeader = true,
    callback = false,
  ) {
    if (utils.isFunction(address)) {
    ...
    const tronAddress = '30' + recovered.substr(2);
    const base58Address = this.lgcyWeb.address.fromHex(tronAddress);

    if (base58Address == this.lgcyWeb.address.fromHex(address))
  ...
  async sign(
    transaction = false,
    privateKey = this.lgcyWeb.defaultPrivateKey,
    useTronHeader = true,
    multisig = false,
    callback = false,
  ) {
    ...
    try {
      if (!multisig) {
        const address = this.lgcyWeb.address
          .toHex(this.lgcyWeb.address.fromPrivateKey(privateKey))
          .toLowerCase();

        if (
          address !==
          transaction.raw_data.contract[0].parameter.value.owner_address.toLowerCase()
        )
  ...
  async sendTransaction(
    ...
    try {
      const address = options.privateKey
        ? this.lgcyWeb.address.fromPrivateKey(options.privateKey)
        : options.address;
      const transaction = await this.lgcyWeb.transactionBuilder.sendTrx(
        to,
        amount,
        address,
      );
  ...
  async sendToken(
    ...
    try {
      const address = options.privateKey
        ? this.lgcyWeb.address.fromPrivateKey(options.privateKey)
        : options.address;
      const transaction = await this.lgcyWeb.transactionBuilder.sendToken(
        to,
        amount,
        tokenID,
        address,
   ...
   * Will give USDL_POWER and TRON Power(voting rights)
  ...
  async freezeBalance(
    ...
    try {
      const address = options.privateKey
        ? this.lgcyWeb.address.fromPrivateKey(options.privateKey)
        : options.address;
      const freezeBalance = await this.lgcyWeb.transactionBuilder.freezeBalance(
        amount,
        duration,
        resource,
        address,
  ...
  async unfreezeBalance(
    ...
    try {
      ...
      const unfreezeBalance =
        await this.lgcyWeb.transactionBuilder.unfreezeBalance(
          resource,
          address,
          receiverAddress,
        );
  ...
  async createProposal(parameters = false, options = {}, callback = false) {
    ...
    try {
      ...
      const createProposal =
        await this.lgcyWeb.transactionBuilder.createProposal(
          parameters,
          address,
        );
      const signedTransaction = await this.sign(
  ...
  async voteProposal(
    ...
    try {
      const address = options.privateKey
        ? this.lgcyWeb.address.fromPrivateKey(options.privateKey)
        : options.address;
      const voteProposal = await this.lgcyWeb.transactionBuilder.voteProposal(
        proposalID,
        isApproval,
        address,
      );
  ...
  async deleteProposal(proposalID = false, options = {}, callback = false) {
    ...
    try {
      ...
      const deleteProposal =
        await this.lgcyWeb.transactionBuilder.deleteProposal(
          proposalID,
          address,
        );
      const signedTransaction = await this.sign(
  ...
  async createSmartContract(options = {}, issuer = {}, callback = false) {
    ...
    if (typeof issuer === 'string') issuer = { privateKey: issuer };
    ...
    issuer = {
      privateKey: this.lgcyWeb.defaultPrivateKey,
      issuer: this.lgcyWeb.defaultAddress.hex,
      ...issuer,
    };
    ...
    try {
      const address = issuer.privateKey
        ? this.lgcyWeb.address.fromPrivateKey(issuer.privateKey)
        : issuer.address;
      const createSmartContract =
        await this.lgcyWeb.transactionBuilder.createSmartContract(
          options,
          address,
        );
      const signedTransaction = await this.sign(
  ...
  async _triggerSmartContract(
    ...
    try {
      const address = issuerAddress.privateKey
        ? this.lgcyWeb.address.fromPrivateKey(issuerAddress.privateKey)
        : issuerAddress.address;
      const _triggerSmartContract =
        await this.lgcyWeb.transactionBuilder._triggerSmartContract(
          contractAddress,
          functionSelector,
          options,
          parameters,
      ...
      const signedTransaction = await this.sign(
        ...
        issuerAddress.privateKey || undefined,
      );
      const result = await this.sendRawTransaction(signedTransaction);
  ...
  async createToken(options = {}, issuer = {}, callback = false) {
    ...
    if (!callback) return this.injectPromise(this.createToken, options, issuer);
    ...
    try {
      const address = issuer.privateKey
        ? this.lgcyWeb.address.fromPrivateKey(issuer.privateKey)
        : issuer.address;
      const createToken = await this.lgcyWeb.transactionBuilder.createToken(
        options,
        address,
      );
      const signedTransaction = await this.sign(
  ...
  async applyForSR(options = {}, url = false, callback = false) {
    ...
    if (!callback) return this.injectPromise(this.applyForSR, options, url);
    ...
    try {
      const address = options.privateKey
        ? this.lgcyWeb.address.fromPrivateKey(options.privateKey)
        : options.address;
      const applyForSR = await this.lgcyWeb.transactionBuilder.applyForSR(
        address,
        url,
      );
      const signedTransaction = await this.sign(
  ...
  async vote(votes = {}, options = {}, callback = false) {
    ...
    if (!callback) return this.injectPromise(this.vote, votes, options);
    ...
    try {
      const address = options.privateKey
        ? this.lgcyWeb.address.fromPrivateKey(options.privateKey)
        : options.address;
      const vote = await this.lgcyWeb.transactionBuilder.vote(votes, address);
      const signedTransaction = await this.sign(
        vote,
        options.privateKey || undefined,
      );
  ...
  async _getReward(
    ...
    this.lgcyWeb[options.confirmed ? 'solidityNode' : 'fullNode']
      .request(
  ...
  async updateAccount(accountName = false, options = {}, callback = false) {
    ...
    try {
      const address = options.privateKey
        ? this.lgcyWeb.address.fromPrivateKey(options.privateKey)
        : options.address;
      const updateAccount = await this.lgcyWeb.transactionBuilder.updateAccount(
        accountName,
        address,
      );
      const signedTransaction = await this.sign(
